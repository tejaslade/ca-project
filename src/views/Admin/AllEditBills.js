...
import { ImageFilterAdmin } from "views/queries/AdminImageBillsFilter/AdminImageBillsFilterQueries";
...
const AllEditBills = () => {
  ...
  const { loading, error, data, refetch } = useQuery(ImageFilterAdmin, {
    variables: {
      user: state_data?.state?.user_name,
      service_name: state_data?.state?.service_name,
    },
  });
  ...
  //   (data?.images?.data?.map((item) => item?.attributes?.service?.data?.attributes?.service_name
